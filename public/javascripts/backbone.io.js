			...
			backend.ready(function() {
				...
				backend.socket.emit('sync', req, function(err, resp) {
					if (err) {
						...
						//restore references when we read an object from database.
						// does not work on initial fetch because we need everything else
						// in local memory to actually restore references.
						collection.restore(resp);
						success(resp);
					}
				});
			});
	...
	var buildBackend = function(collection) {
		...
		backend.socket.emit('listen', function(options) {
			...
			backend.socket.on('synced', function(method, resp) {
				...
				window.collection.restore(resp);
